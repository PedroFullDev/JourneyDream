...
floor_height_scenario1 = (
    130  # Altura do chão do cenário 1 - garante o personagem no solo
)
...
scenario1_background_parts = [
    ...
    "image5_background_scenario_introduction",
    "image6_background_scenario_introduction",
    "image7_background_scenario_introduction",
    "background_scenario1_part1",
    "background_scenario1_part2",
    "background_scenario1_part3",
    "background_scenario1_final_items",
]
...
buttons = [
    {"text": "Iniciar", "x": 320, "y": 300, "action": "start"},
    {"text": "Sair", "x": 320, "y": 380, "action": "exit"},
    {"text": "Som: Ligado", "x": 320, "y": 460, "action": "toggle_sound"},
]
...
def toggle_sound():
    ...
    for b in buttons:
        if b["action"] == "toggle_sound":
            b["text"] = "Som: Ligado" if sound_enabled else "Som: Desligado"


def update():
    global hero_walking, hero_jumping, hero_direction, current_scenario1_index, hero_velocit_y, hero_attacking, hero_attack_cooldown, event_step, event_active, game_state, hero_won, victory_message_shown, hero_exiting, hero_alive
    ...
    if game_state == "menu":
        if keyboard.K_1:
            ...
            exit()
        elif keyboard.K_3:  # Alterna sons
            toggle_sound()
        return

    if hero_exiting and hero_alive:
        hero.x += 3  # vai para o lado esquerdo
        if hero.left > WIDTH:  # saiu da tela
            hero_exiting = False
            hero_alive = False
            clock.schedule_unique(lambda: set_victory_message(), 0.8)
        return
    ...
    if (
        event_active
    ):  # Enquanto ativo, ignora as tentativas movimentação apenas do personagem
    ...
    if hero.x >= WIDTH:
        if current_scenario1_index < len(scenario1_background_parts) - 1:
            if current_scenario1_index + 1 == 10:
                all_zombies_dead = all(
                    len(zombies) == 0
                    for idx, zombies in zombies_by_scenario.items()
                    if idx < 10
                )
    ...
    if event_active and keyboard.RETURN:
        event_step = event_step + 1
        if event_step > 4:
            event_active = False
            hero.image = hero_stopped_right
    ...
    for zombie in current_zombies:
        if abs(zombie.y - hero.y) < 50:
            if (
                hero_direction == "right"
                and zombie.x > hero.x
                and zombie.x - hero.x <= hero_attack_range
            ):
                stun_zombie(zombie)
            elif (
                hero_direction == "left"
                and zombie.x < hero.x
                and hero.x - zombie.x <= hero_attack_range
            ):
                stun_zombie(zombie)
    ...
    for zombie in current_zombies:
        if hero.colliderect(zombie):
            ...
            clock.schedule_unique(
                lambda: restore_zombie_speed(zombie, original_zombie_speed), 1.0
            )
            clock.schedule_unique(
                remove_invulnerability, 0.5
            )  # Fica invulnerável por 0,5 segundos
            if hero_health <= 0:
                print(f"Você morreu!")
                hero_alive = False
...
def draw():
    screen.clear()
    if game_state == "menu":
        screen.blit("menu_main.png", (0, 0))

        mouse_pos = (0, 0)
        if hasattr(mouse, "x") and hasattr(mouse, "y"):
            mouse_pos = (mouse.x, mouse.y)

        for b in buttons:
            button_rect = Rect((b["x"] - 20, b["y"] - 10), (200, 50))
            is_hovered = button_rect.collidepoint(mouse_pos)

            color_bg = (50, 50, 50) if not is_hovered else (200, 200, 0)
            color_text = (255, 255, 255) if not is_hovered else (0, 0, 0)

            screen.draw.filled_rect(button_rect, color_bg)
            screen.draw.rect(button_rect, (255, 255, 255))
            screen.draw.text(
                b["text"], center=button_rect.center, fontsize=36, color=color_text
            )
        return
    ...
    if victory_message_shown:
        screen.draw.text("Você venceu!", (200, 200), color="yellow", fontsize=60)
    ...
    if game_over_message:
        screen.draw.text(game_over_message, (200, 200), color="red", fontsize=60)
...
def draw_zombie_health_bar(zombie):  # Barra de vida dos zumbis
    ...
    screen.draw.filled_rect(
        Rect((bar_x, bar_y), (bar_width, bar_height)), (100, 100, 100)
    )  # Fundo cinza da barra de vida
    screen.draw.filled_rect(
        Rect((bar_x, bar_y), (health_bar_width, bar_height)), (200, 0, 0)
    )  # Barra vermelha de vida
    screen.draw.rect(
        Rect((bar_x, bar_y), (bar_width, bar_height)), (0, 0, 0)
    )  # Borda da barra de vida


def on_key_down(key):
    global current_scenario1_index, hero_exiting, hero_won, event_step

    if key == keys.RETURN and current_scenario1_index < 6:
        current_scenario1_index = current_scenario1_index + 1

    if event_step == 2 and key == keys.RETURN:
        hero_exiting = True
        hero_won = True
        hero.image = hero_stopped_right
        sounds.victory.stop()
        music.stop()
        if sound_enabled:
            music.play("music_credits.wav")
            music.set_volume(0.5)


def set_victory_message():
    global victory_message_shown
